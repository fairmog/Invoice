...
class CacheError extends Error {
  constructor(message, merchantId) {
    super(message);
...
class SimpleDatabase {
  constructor() {
    ...
    // Performance optimization: In-memory cache for frequent operations
    this.cache = {
      merchantSettings: new Map(),
  ...
  runMigrations() {
    ...
    if (!this.data.schema_version) {
      this.data.schema_version = 0;
    }
    
    if (!this.data.migration_log) {
      this.data.migration_log = [];
    }
    ...
    for (const migration of migrations) {
      if (this.data.schema_version < migration.version) {
        console.log(`🔄 Running migration ${migration.version}: ${migration.description}`);
        
        try {
          migration.up(this.data);
          this.data.schema_version = migration.version;
          
          // Log successful migration
          this.data.migration_log.push({
    ...
    if (migrationRan) {
      console.log(`✅ Database migrations completed. Current schema version: ${this.data.schema_version}`);
      this.saveData();
    }
  }
  ...
  getMigrationDefinitions() {
    return [
      {
        version: 1,
        description: 'Add merchant_business_settings for security isolation',
        up: (data) => {
          if (!data.merchant_business_settings) {
            data.merchant_business_settings = {};
          }
        },
        down: (data) => {
          // Rollback: remove merchant_business_settings (use with caution)
          delete data.merchant_business_settings;
        }
      },
      {
        version: 2,
        description: 'Add performance monitoring fields',
        up: (data) => {
        ...
        down: (data) => {
          delete data.performance_metrics;
        }
      }
      // Future migrations can be added here with incrementing version numbers
    ];
  }

  // Utility: Get migration status for debugging
  getMigrationStatus() {
    return {
      currentVersion: this.data.schema_version || 0,
      availableMigrations: this.getMigrationDefinitions().length,
      migrationHistory: this.data.migration_log || []
    };
  }

  // Logging utility for structured logging
  logOperation(operation, merchantId, details = {}, level = 'info') {
    const logEntry = {
      timestamp: new Date().toISOString(),
      operation,
      merchantId,
    ...
    if (this.data.performance_metrics && this.data.performance_metrics.database_operations) {
      // Keep only last 100 operations to prevent memory bloat
      this.data.performance_metrics.database_operations.push(logEntry);
      if (this.data.performance_metrics.database_operations.length > 100) {
        this.data.performance_metrics.database_operations.shift();
      }
    }

    // Console logging for development
    const logMessage = `${level.toUpperCase()}: ${operation} - Merchant: ${merchantId}`;
    if (level === 'error') {
  ...
  // Security & Performance: Centralized merchant validation with enhanced error handling
  validateMerchantId(merchantId, operation = 'access') {
    if (!merchantId || typeof merchantId !== 'string' || merchantId.trim() === '') {
      const error = new ValidationError(
        `Merchant ID is required for ${operation}. Please provide a valid merchant identifier.`,
        'merchantId',
        merchantId
      );
      this.logOperation('validation_failed', merchantId || 'unknown', { 
        operation, 
        error: error.message 
      }, 'error');
      throw error;
    ...
    this.logOperation('validation_success', cleanMerchantId, { operation }, 'info');
    return cleanMerchantId;
  }

  // Cache Management: Check if cached data is still valid
  isCacheValid(merchantId) {
    const lastCacheTime = this.cache.lastCacheTime.get(merchantId);
    if (!lastCacheTime) return false;
    return (Date.now() - lastCacheTime) < this.cacheTimeout;
  }

  // Cache Management: Get from cache or return null
  getCachedMerchantSettings(merchantId) {
    if (this.isCacheValid(merchantId)) {
      // Track cache hit
      if (this.data.performance_metrics) {
        this.data.performance_metrics.cache_stats.hits++;
      }
      return this.cache.merchantSettings.get(merchantId);
    }
    
    // Track cache miss
    if (this.data.performance_metrics) {
      this.data.performance_metrics.cache_stats.misses++;
    }
    return null;
  }

  // Cache Management: Store in cache
  setCachedMerchantSettings(merchantId, settings) {
    this.cache.merchantSettings.set(merchantId, { ...settings });
    this.cache.lastCacheTime.set(merchantId, Date.now());
  }

  // Cache Management: Invalidate cache for specific merchant
  invalidateMerchantCache(merchantId) {
    this.cache.merchantSettings.delete(merchantId);
    this.cache.lastCacheTime.delete(merchantId);
  }

  // Utility: Get default merchant settings template
  ...
  async getOrder(id) {
    ...
    console.log(`DEBUG: getOrder(${id}) - Found order:`, order);
    console.log(`DEBUG: getOrder(${id}) - Found ${items.length} items:`, items);
  ...
  // Business Settings CRUD operations - Optimized with caching and error handling
  async getBusinessSettings(merchantId = null) {
    try {
      const validatedMerchantId = this.validateMerchantId(merchantId, 'business settings access');
      
      // Try cache first for performance
      const cachedSettings = this.getCachedMerchantSettings(validatedMerchantId);
      if (cachedSettings) {
        this.logOperation('get_business_settings', validatedMerchantId, { source: 'cache' });
        return cachedSettings;
      }
      
      // Ensure merchant settings exist (lazy initialization)
      await this.ensureMerchantSettings(validatedMerchantId);
      
      const settings = this.data.merchant_business_settings[validatedMerchantId];
      
      if (!settings) {
        throw new DatabaseError(
          'Failed to retrieve business settings after initialization',
      ...
      this.setCachedMerchantSettings(validatedMerchantId, settings);
      
      this.logOperation('get_business_settings', validatedMerchantId, { 
        source: 'database',
      ...
      this.logOperation('get_business_settings_error', merchantId || 'unknown', { 
        error: error.message,
  ...
  // Utility: Ensure merchant settings exist without triggering auto-save on reads
  async ensureMerchantSettings(merchantId) {
    if (!this.data.merchant_business_settings[merchantId]) {
      console.log(`🔧 Initializing settings for merchant: ${merchantId}`);
      this.data.merchant_business_settings[merchantId] = this.getDefaultMerchantSettings();
      this.saveData(); // Only save when actually creating new settings
    }
  }

  async updateBusinessSettings(settings, merchantId = null) {
    const validatedMerchantId = this.validateMerchantId(merchantId, 'business settings update');

    // Get existing settings for this merchant
    const existingSettings = await this.getBusinessSettings(validatedMerchantId);
    ...
    // Update merchant-specific settings with enhanced validation
    ...
    this.data.merchant_business_settings[validatedMerchantId] = updatedSettings;

    // Invalidate cache and update with new data
    this.invalidateMerchantCache(validatedMerchantId);
    this.setCachedMerchantSettings(validatedMerchantId, updatedSettings);
  ...
  // Premium Branding Helper Methods - Optimized with caching
  async isPremiumActive(merchantId = null) {
    const validatedMerchantId = this.validateMerchantId(merchantId, 'premium status check');
    
    try {
      const businessSettings = await this.getBusinessSettings(validatedMerchantId);
      return businessSettings?.premiumActive === true;
    } catch (error) {
      console.error(`Error checking premium status for merchant ${validatedMerchantId}:`, error);
      return false;
    }
  }

  async getPremiumBrandingSettings(merchantId = null) {
    const validatedMerchantId = this.validateMerchantId(merchantId, 'premium branding settings access');
    ...
    try {
      ...
      return {
        ...
        customColors: businessSettings.customColors || {
          primary: '#6366f1',
          secondary: '#4f46e5'
        }
      };
    } catch (error) {
      console.error(`Error fetching premium branding settings for merchant ${validatedMerchantId}:`, error);
      return null;
    }
  }
