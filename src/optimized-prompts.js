...
Return JSON only:
{
  ...
  generateInvoice: (orderData, merchantConfig, paymentOptions = {}) => {
    try {
      ...
      // Down payment due date - use today for immediate payment, or calculated date for scheduled
      const downPaymentDueDate = hasImmediateDP ? today : 
        new Date(new Date(today).getTime() + downPaymentDays * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
