...
Return JSON only:
{
  ...
  generateInvoice: (orderData, merchantConfig, paymentOptions = {}) => {
    try {
      const today = new Date().toISOString().split('T')[0];
      // Only generate due date if specified in order data or merchant config
      ...
      // Down payment due date should match main invoice due date
      const downPaymentDueDate = dueDate || today;
...
Return JSON only:
{
  "invoice": {
    ...
    "payment": {
      ...
      "paymentInstructions": ${enablePaymentSchedule ? `"Down payment ${downPaymentPercentage}% due in ${downPaymentDays} days, remaining balance due in ${finalPaymentDays} days"` : '"Pembayaran dalam 30 hari"'},
  ...
  validateInvoice: (invoice) => `Validate this invoice:
${JSON.stringify(invoice, null, 2)}

Check:
- Required fields present
- Calculations correct (subtotal, tax 11%, total)
- Valid Indonesian formats
- Dates not in future

Return JSON only:
{
  "isValid": boolean,
  "errors": [{"field": "fieldName", "error": "description", "severity": "error"}],
