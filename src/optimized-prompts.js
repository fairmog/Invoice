...
Return JSON only:
{
  ...
  generateInvoice: (orderData, merchantConfig, paymentOptions = {}) => {
    try {
      ...
      // Pre-calculate payment schedule values based on type
...
Return JSON only:
{
  "invoice": {
    ...
    "calculations": {
      ...
      "discountValue": ${orderData.orderDetails?.discount || 0},
    ...
    ${enablePaymentSchedule ? `"paymentSchedule": {
      ...
      "downPayment": {
        ...
        "percentage": ${downPaymentType === 'percentage' ? downPaymentValue : Math.round((downPaymentAmount / grandTotal) * 100)},
    ...
    "payment": {
      "paymentTerms": ${enablePaymentSchedule ? (downPaymentType === 'percentage' ? `"DOWN_PAYMENT_${downPaymentValue}%"` : `"DOWN_PAYMENT_FIXED"`) : '"NET_30"'},
      "paymentMethods": ${JSON.stringify(getEnabledPaymentMethodsIndonesian(merchantConfig))},
      "paymentInstructions": ${enablePaymentSchedule ? (downPaymentType === 'percentage' ? `"Down payment ${downPaymentValue}% due immediately, remaining balance due as agreed"` : `"Down payment amount due immediately, remaining balance due as agreed"`) : '"Pembayaran segera"'},
