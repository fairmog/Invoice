...
- Optimize for both print and digital viewing`;
  ...
  async validateAndRecalculateInvoice(invoiceData) {
    try {
      ...
      // DATA NORMALIZATION: Ensure all items have consistent field structure
      // Fix for issue where productName, unitPrice, lineTotal show as ":" instead of values
      items = items.map((item, index) => {
        const normalizedItem = {
          // Handle productName field with multiple fallback formats
          productName: item.productName || item.product_name || item.name || item.productname || `Product ${index + 1}`,
          
          // Handle quantity field
          quantity: parseInt(item.quantity || item.qty || 1),
          
          // Handle unitPrice field with multiple fallback formats
          unitPrice: parseFloat(item.unitPrice || item.unit_price || item.price || item.unitprice || 0),
          
          // Handle lineTotal field with multiple fallback formats
        ...
        // Calculate line total if missing or zero
      ...
      // Update the invoice with normalized items
      invoiceData.invoice.items = items;
